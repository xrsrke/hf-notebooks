...
def fetch_data(metric_filters, x_axis_options, name_filter):
    ...
    for run in runs:
        ...
        summary = run.summary
        steps = summary.get('iteration_step')
        ...
        if batch_size:
            run_data = {
                ...
                'steps': steps,
                'name': run.name,
                'iteration_step': summary.get('iteration_step'),
...
if submit_button:
    ...
    st.sidebar.success(f"Data loaded: {len(st.session_state.df)} runs")
...
with internal_state_tab:
    ...
    # Display current filters and x-axis options and allow removal
